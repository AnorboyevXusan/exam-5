...
const isMarriedInp = document.querySelector(".isMarried")
  ...
  return `
    <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-3">
      <div class="card bg-danger-subtle">
        ...
        />
        <div class="card-body">
          <h4 class="card-title">${fristName} ${lastName}</h4>
          <a href="${email}">${email}</a>
          <div class="form-check">
            <span>${isMarried ? 'merried' : 'not merried'}<span/>
          ...
          <div>
            ...
            <a href="product.html?categoryId=${id}" class="btn btn-primary">Student ${id}</a>
          </div>
        </div>
      </div>
    </div>
...
async function getCategories() {
  try {
    let params = !isMarriedInp.checked ? select.value === 'fristName' ? { fristName: search } : { lastName: search } : { isMarried: isMarried }
...
isMarriedInp.addEventListener('click', function () {
  isMarried = !isMarried
  ...
  try {
    ...
    if (this.checkValidity()) {
      let category = {
        ...
        phoneNuber: this.phoneNuber.value,
...
// // Фильтрация по isMarried
// const filteredByMarried = people.filter(categoriesData => categoriesData.isMarried === true);

// // Поиск по firstName
// const searchName = 'John'; // Замените на значение, которое вы ищете
// const filteredByName = categoriesData.filter(person =>
//   person.firstName.toLowerCase().includes(searchName.toLowerCase())
// )

// // Сортировка по lastName (по возрастанию)
// const sortedByLastNameAsc = categoriesData.sort((a, b) => a.lastName.localeCompare(b.lastName));

// // Сортировка по lastName (по убыванию)
// const sortedByLastNameDesc = categoriesData.sort((a, b) => b.lastName.localeCompare(a.lastName));